...
- **`Packages/com.hk.modtools.compatpatcher/Editor/CompatPatcher/`** (experimental, no tag yet) — the Compatibility Patcher; depends on `shared` only.
...
**Group:** `CompatPatcherWindow.cs`, `CompatCompareWindow.cs`, `CompatBundleMounts.cs`, `LiveElementBuilder.cs`, `UnityYaml.cs`, `ModReader.cs`, `ConflictAnalyzer.cs`, `PatchBuilder.cs`, `Sidecar.cs`, `LoadOrderValidator.cs`, `DiffGui.cs` (all under `com.hk.modtools.compatpatcher`'s `Editor/CompatPatcher/`)
...
2. **CompatPatcher package (`com.hk.modtools.compatpatcher`):** `CompatPatcher/CompatPatcherWindow.cs`, `CompatPatcher/CompatCompareWindow.cs`, `CompatPatcher/CompatBundleMounts.cs`, `CompatPatcher/LiveElementBuilder.cs`, `CompatPatcher/UnityYaml.cs`, `CompatPatcher/ModReader.cs`, `CompatPatcher/ConflictAnalyzer.cs`, `CompatPatcher/PatchBuilder.cs`, `CompatPatcher/Sidecar.cs`, `CompatPatcher/LoadOrderValidator.cs`, `CompatPatcher/DiffGui.cs`
